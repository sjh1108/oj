# Judge0에 PyPy 3 추가하기 (적용 완료 — 현재 구성에서는 쓰지 않음)

> **이 절차는 더 이상 필요 없다.** Judge0 공식 이미지가 amd64 전용이라 arm64 박스로 이전하면서
> 채점기를 [`judge-runner`](../../judge-runner/README.md)로 바꿨고, PyPy는 샌드박스 이미지에
> 처음부터 들어 있다. 아래는 x86 박스에서 정품 Judge0를 다시 쓰게 될 때를 위한 기록이다.
