...
즉 **메모리 제한이 500MB를 넘는 문제는 전부 실패**한다 — 백준에서 흔한 **512MB(524288KB)** 도 포함이다.
...
## 박스에서 할 일 (상한을 실제로 올릴 때만, 수동)

문제의 메모리 제한을 액면 그대로 채점하고 싶다면 **박스 상한과 앱 설정을 같이** 올려야 한다.
앱 설정만 올리면 다시 422가 나고, 박스만 올리면 앱이 계속 예전 값으로 클램프한다.
...
그 다음 API 박스 `/opt/algoj/.env`에 `JUDGE0_MAX_MEMORY_LIMIT_KB=524288`을 넣고 재배포.

> **박스 메모리를 먼저 볼 것.** 현재 Judge0 박스는 ≈2GB에 Judge0 + RabbitMQ가 이미
> 1.2GB 남짓을 쓴다. 512MB(524288)까지는 여유가 있지만 **1GB(1048576)는 권장하지 않는다** —
