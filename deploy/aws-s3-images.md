...
### 왜 700KB인가 — nginx 1MB 제한과의 관계

업로드 요청은 nginx(기본 `client_max_body_size` 1MB)를 지나므로, base64가
원본의 4/3로 팽창하는 것을 역산해 원본 제한을 700KB로 잡았다:
원본 700KB(716,800B) → base64 약 956KB → JSON 오버헤드 포함해도 1MB
(1,048,576B) 아래로 약 9% 여유를 두고 통과한다. nginx 설정은 건드리지 않는다
(프로젝트 규칙 — 업로드 문제는 앱 레벨에서 해결).

`.md` 일괄 업로드도 안전하다: `.md` 파일 자체는 브라우저에서 파싱될 뿐 서버로
전송되지 않고, 이미지는 **한 장당 요청 하나씩** 올라가므로 이미지가 몇 장이든
요청당 크기는 1MB 아래다. 프론트는 700KB를 넘는 파일을 요청 전에 걸러
명확한 에러 메시지를 보여준다 (서버 검증 I001이 최종 방어선).
