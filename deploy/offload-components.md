# 컴포넌트 분리 (단일 박스 부하 오프로드) — 완료 기록

> **아래 세 이전(MySQL→RDS, Judge0→JJ, RabbitMQ→JJ)은 전부 끝났다.** 이 문서는 왜·어떻게
> 옮겼는지의 **기록**이고, 남은 할 일이 아니다. 현재 구성과 운영 절차는 [`README.md`](README.md),
> 이중화는 [`redundancy.md`](redundancy.md)를 본다.

원래 배포는 **한 박스(Lightsail ≈2GB)에 nginx·API·MySQL·RabbitMQ·Judge0·봇을 전부** 올렸다.
상태가 있거나 무거운 컴포넌트를 별도 인스턴스/관리형 서비스로 빼면 **박스 메모리가 확보되고
...
2. **분리해도 남는 안전망**: 스왑·힙 캡·SerialGC는 그대로 둬도 무방.
   MySQL·Judge0·RabbitMQ를 다 빼서 OJ에는 nginx+API+봇만 남았다. 배포 다운타임은
   **한 박스 겹침(블루-그린)이 아니라 OJ+EOJ 교차 롤링**으로 해결됐다 —
   `deploy/redundancy.md`, 운영 절차는 `deploy/README.md`의 *무중단 배포* 절 참고.
3. **비용/복잡도**: 박스가 늘면 관리 지점·요금도 는다. RDS/Judge0 분리는
   **백업·격리·독립 스케일**이 필요할 때 값어치가 있다. (당시 대안이던 Lightsail 4GB 증설은
   이중화로 무중단을 얻으면서 목적이 사라져 **하지 않기로 결정**했다.)
